...
const SquatClassifier = () => {
  ...
  const STAGE_UP_THRESHOLD = 160;
  const KNEE_OVER_TOE_THRESHOLD = 0.15;
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        // Load the squat classifier model
        try {
          const model = await tf.loadLayersModel('/squat-model/saved_model/squat_model/model.json');
          modelRef.current = model;
          
          // Load the scaler data
          const scalerResponse = await fetch('/squat-model/saved_model/squat_model_metadata.json');
  ...
  // Process landmarks for squat classification
  const processLandmarks = async (landmarks: any[]) => {
    ...
    try {
      ...
      setClassification({
        position: predictedClass === 0 ? 'down' : 'up',
        confidence
      });
      
      // 5. Analyze squats for counting and form feedback
  ...
  const analyzeSquats = (landmarks: any[], predictedClass: number) => {
    ...
    try {
      ...
      if (avgKneeAngle < STAGE_DOWN_THRESHOLD && stageRef.current === 'up') {
        stageRef.current = 'down';
        setCounter(prev => prev + 1);
      } else if (avgKneeAngle > STAGE_UP_THRESHOLD && stageRef.current === 'down') {
        stageRef.current = 'up';
      }
      ...
      if (avgKneeOverToe > KNEE_OVER_TOE_THRESHOLD) {
        newWarnings.push("KNEES EXTENDING OVER TOES");
    ...
    //   if ((stageRef.current === 'down' && predictedClass === 1) || 
    //       (stageRef.current === 'up' && predictedClass === 0)) {
    //     newWarnings.push("POSITION MISMATCH WITH MODEL PREDICTION");
    //   }
      ...
      {isRunning && (
        <div className="absolute top-4 right-4 space-y-2">
          <div className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${
            ...
            {classification.position === 'up' ? 'UP POSITION' : 'DOWN POSITION'}
            <span className="text-xs">({Math.round(classification.confidence * 100)}%)</span>
          </div>
